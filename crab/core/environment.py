...
from .models import Action, ClosedAction, EnvironmentConfig, VMEnvironmentConfig
from .vagrant_manager import get_vagrant_manager
...
class VMEnvironment(Environment):
    def __init__(
        ...
        observation_space: list[ClosedAction],
        description: str = "",
        reset: Optional[Action] = None,
        remote_url: Optional[str] = None,
        benchmark_name: str = "test_benchmark",
        environment_name: str = "test_environment",
    ) -> None:
        parsed_url = urlparse(remote_url)
        vagrant_manager = get_vagrant_manager()
        vagrant_manager.start_vm(
            name, parsed_url.hostname, parsed_url.port, benchmark_name, environment_name
        )

        # Call the superclass constructor with the remote_url
        super().__init__(
            name=name,
    ...
    # Implement any other VM-specific methods here


def create_environment(config):
    if isinstance(config, VMEnvironmentConfig):
        # Extract the inside_environment as a dict and create a VMEnvironment
        env_config = dict(config.inside_environment)
        env_config["remote_url"] = config.remote_url
        return VMEnvironment(**env_config)
